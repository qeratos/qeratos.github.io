...
3) Почти в самом конце затаился указатель на количество ядер нашего процессора, поле с наименованием **`NumberOfProcessors**, что может помочь злоумышленникам проводить проверку на запуск в контролируемой среде. ![NumberOfProcessors](assets/post_img/hidden_peb/numb_of_cpu.png)
